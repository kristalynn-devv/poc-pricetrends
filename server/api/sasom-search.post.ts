...
const FIELD_DESCRIPTIONS: Record<string, string> = {
  ...
  brand: 'แบรนด์ เช่น Celine, Gucci, Louis Vuitton',
  model: 'รุ่น เช่น Triomphe, Marmont',
  itemType: 'ประเภทสินค้า เช่น แว่นตา, กระเป๋า, เสื้อผ้า',
...
function buildFilename(index: number, url: string): string {
  ...
  return `${date}_108_SAS_${slug}.jpg`
}
...
export default defineEventHandler(async (event) => {
  ...
  const extractPrompt = `คุณคือผู้ช่วยสกัดข้อมูลสินค้าแบรนด์เนมมือสอง จากภาพหน้าเว็บ sasom.co.th
...
${schemaText}
  ...
  const emit = (level: 'info' | 'warn' | 'error', msg: string, data?: unknown) => {
    ...
    const prefix = `[sasom][${level.toUpperCase()}]`
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const productUrlRe = /sasom\.co\.th\/th\/[^/]+\/[A-Z0-9]+-[A-Z0-9]+\//
          const allProductUrls = [...new Set(allHrefs.filter((h) => productUrlRe.test(h)))]
          ...
          emit('info', `Filtered URLs`, { matched: matched.length, total: allProductUrls.length })
          listingUrls = matched.slice(0, limit)
        } finally {
          await ctx.close()
        }
      } catch (err) {
      ...
      if (listingUrls.length === 0) {
        ...
        await appendLog({ timestamp: new Date().toISOString(), source: 'sasom', url: searchUrl, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, itemsExtracted: 0, error: 'No listing URLs found', errorType: null }).catch(() => {})
      ...
      for (let i = 0; i < listingUrls.length; i++) {
        ...
        try {
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'sasom', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: isTimeout ? 504 : 500, screenshotFile: null, itemsExtracted: null, error: msg, errorType: isTimeout ? 'timeout' : 'screenshot' }).catch(() => {})
        ...
        try {
          ...
          try {
            ...
            await Promise.all([
              appendLog({ timestamp: ts, source: 'sasom', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: result.items.length, error: null, errorType: null }).catch(() => {}),
              result.items.length > 0 ? saveResult({ timestamp: ts, source: 'sasom', url, categoryId, screenshotFile: filename, items: result.items }).catch((e) => emit('warn', 'saveResult failed', String(e))) : Promise.resolve(),
            ])
          } catch {
            result.raw = text
            result.extractOk = false
            emit('warn', `[${i + 1}] Gemini response not valid JSON`, { preview: text.slice(0, 120) })
            await appendLog({ timestamp: new Date().toISOString(), source: 'sasom', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: 0, error: `JSON parse failed: ${text.slice(0, 120)}`, errorType: 'parse' }).catch(() => {})
          }
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err)
          result.error = (result.error ?? '') + `extract: ${msg}`
          emit('error', `[${i + 1}] Gemini failed`, { msg })
          await appendLog({ timestamp: new Date().toISOString(), source: 'sasom', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, itemsExtracted: null, error: msg, errorType: 'extraction' }).catch(() => {})
        }
