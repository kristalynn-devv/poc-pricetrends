...
export default defineEventHandler(async (event) => {
  ...
  const extractPrompt = `คุณคือผู้ช่วยสกัดข้อมูลพระ/วัตถุมงคลจากภาพหน้าเว็บ uauction.uamulet.com
...
ราคาในหน้านี้คือราคาประมูล ให้ดึงราคาปัจจุบันหรือราคาสูงสุดของการประมูล
  ...
  ;(async () => {
    ...
    try {
      ...
      try {
        ...
        const allHrefs = await page.locator('a[href*="AuctionDetail.aspx"]').evaluateAll(
          (els) => [...new Set((els as HTMLAnchorElement[]).map((a) => a.href))]
        )
        const detailUrls = allHrefs.slice(0, limit)
        emit('info', `Found ${allHrefs.length} auction links, processing ${detailUrls.length}`)

        if (detailUrls.length === 0) {
          emit('warn', 'No auction links found')
          await appendLog({ timestamp: new Date().toISOString(), source: 'uauction', url: SEARCH_URL, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, itemsExtracted: 0, error: 'No auction links found', errorType: null }).catch(() => {})
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                await Promise.all([
                  appendLog({ timestamp: ts, source: 'uauction', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: result.items.length, error: null, errorType: null }).catch(() => {}),
                  result.items.length > 0 ? saveResult({ timestamp: ts, source: 'uauction', url, categoryId, screenshotFile: filename, items: result.items }).catch((e) => emit('warn', 'saveResult failed', String(e))) : Promise.resolve(),
                ])
              } catch {
                result.raw = text
                result.extractOk = false
                emit('warn', `[${i + 1}] Gemini response not valid JSON`, { preview: text.slice(0, 120) })
                await appendLog({ timestamp: new Date().toISOString(), source: 'uauction', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: 0, error: `JSON parse failed: ${text.slice(0, 120)}`, errorType: 'parse' }).catch(() => {})
              }
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err)
              result.error = `extract: ${msg}`
              emit('error', `[${i + 1}] Gemini failed`, { msg })
              await appendLog({ timestamp: new Date().toISOString(), source: 'uauction', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, itemsExtracted: null, error: msg, errorType: 'extraction' }).catch(() => {})
            }
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err)
            ...
            await appendLog({ timestamp: new Date().toISOString(), source: 'uauction', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: isTimeout ? 504 : 500, screenshotFile: null, itemsExtracted: null, error: msg, errorType: isTimeout ? 'timeout' : 'screenshot' }).catch(() => {})
          }
