...
const SEARCH_BASE = 'https://shopee.co.th/search'
...
const FIELD_DESCRIPTIONS: Record<string, string> = {
  itemType: 'ประเภทสินค้า เช่น เครื่องเป่าลม, สว่าน | null',
  brand: 'แบรนด์ เช่น Bosch, Makita, Kress | null',
  model: 'รุ่น เช่น GBH 2-26, DHP481 | null',
...
function buildFilename(index: number, categoryId: string): string {
  ...
  return `${date}_${categoryId}_SHP_${String(index + 1).padStart(2, '0')}.jpg`
}
...
export default defineEventHandler(async (event) => {
  ...
  if (template && Object.keys(template).length > 0) {
    ...
    const keys = CATEGORY_FIELDS[categoryId] ?? ['itemType', 'brand', 'model', 'price', 'currency']
  ...
  const extractPrompt = `คุณคือผู้ช่วยสกัดข้อมูลสินค้าจากภาพหน้าเว็บ shopee.co.th
...
${schemaText}
  ...
  const emit = (level: 'info' | 'warn' | 'error', msg: string, data?: unknown) => {
    ...
    const prefix = `[shopee][${level.toUpperCase()}]`
  ...
  const searchUrl = `${SEARCH_BASE}?keyword=${encodeURIComponent(query)}`
  ...
  ;(async () => {
    ...
    try {
      ...
      try {
        ...
        // Take a debug screenshot of the search page to diagnose bot-block or empty results
        const debugBuf = await page.screenshot({ type: 'jpeg', quality: 70, fullPage: false })
        await writeFile(join(screenshotDir, `_shopee_search_debug_${Date.now()}.jpg`), debugBuf)
        emit('info', 'Saved debug search screenshot')

        // Shopee product URLs: shopee.co.th/<slug>-i.<shopId>.<itemId>
        const allHrefs = await page.locator('a[href*="-i."]').evaluateAll(
          (els) => [...new Set((els as HTMLAnchorElement[]).map((a) => a.href).filter((h) => /shopee\.co\.th\/.+-i\.\d+\.\d+/.test(h)))]
        )
        emit('info', `Raw link count before dedup`, { count: allHrefs.length })
        ...
        if (detailUrls.length === 0) {
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'shopee', url: searchUrl, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, itemsExtracted: 0, error: 'No product links found', errorType: null }).catch(() => {})
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                await Promise.all([
                  appendLog({ timestamp: ts, source: 'shopee', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: result.items.length, error: null, errorType: null }).catch(() => {}),
                  result.items.length > 0 ? saveResult({ timestamp: ts, source: 'shopee', url, categoryId, screenshotFile: filename, items: result.items }).catch((e) => emit('warn', 'saveResult failed', String(e))) : Promise.resolve(),
                ])
              } catch {
                result.raw = text
                result.extractOk = false
                emit('warn', `[${i + 1}] Gemini response not valid JSON`, { preview: text.slice(0, 120) })
                await appendLog({ timestamp: new Date().toISOString(), source: 'shopee', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: 0, error: `JSON parse failed: ${text.slice(0, 120)}`, errorType: 'parse' }).catch(() => {})
              }
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err)
              result.error = `extract: ${msg}`
              emit('error', `[${i + 1}] Gemini failed`, { msg })
              await appendLog({ timestamp: new Date().toISOString(), source: 'shopee', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, itemsExtracted: null, error: msg, errorType: 'extraction' }).catch(() => {})
            }
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err)
            ...
            await appendLog({ timestamp: new Date().toISOString(), source: 'shopee', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: isTimeout ? 504 : 500, screenshotFile: null, itemsExtracted: null, error: msg, errorType: isTimeout ? 'timeout' : 'screenshot' }).catch(() => {})
          }
