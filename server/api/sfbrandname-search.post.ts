...
const SFB_BASE = 'https://sfbrandname.com'
...
const FIELD_DESCRIPTIONS: Record<string, string> = {
  ...
  brand: 'แบรนด์ เช่น Celine, Gucci, Louis Vuitton, Hermes',
  model: 'รุ่น เช่น Triomphe, Marmont, Birkin',
  itemType: 'ประเภทสินค้า เช่น กระเป๋า, แว่นตา, เสื้อผ้า, รองเท้า',
  year: 'ปีผลิต เช่น 2023, 2024 | null',
}
...
// Product detail URLs match /co<digits> pattern
const PRODUCT_URL_RE = /sfbrandname\.com\/co\d+/
...
function buildFilename(index: number, url: string): string {
  ...
  return `${date}_108_SFB_${slug}.jpg`
}
...
export default defineEventHandler(async (event) => {
  ...
  if (template && Object.keys(template).length > 0) {
    ...
    const keys = CATEGORY_FIELDS[categoryId] ?? ['itemType', 'brand', 'model', 'price', 'currency', 'year', 'condition']
  ...
  const extractPrompt = `คุณคือผู้ช่วยสกัดข้อมูลสินค้าแบรนด์เนมมือสอง จากภาพหน้าเว็บ sfbrandname.com
...
${schemaText}
  ...
  const emit = (level: 'info' | 'warn' | 'error', msg: string, data?: unknown) => {
    ...
    const prefix = `[sfbrandname][${level.toUpperCase()}]`
  ...
  ;(async () => {
    ...
    try {
      // Try site search first; fall back to /search?q= pattern
      const searchUrl = `${SFB_BASE}/search?type=product&q=${encodeURIComponent(query)}`
      emit('info', `Starting search`, { query, categoryId, limit, searchUrl })
      ...
      try {
        ...
        try {
          ...
          listingUrls = [...new Set(allHrefs.filter((h) => PRODUCT_URL_RE.test(h)))].slice(0, limit)
          emit('info', `Found ${listingUrls.length} product URLs from search`)

          if (listingUrls.length === 0) {
            // sfbrandname may use a different search path — try fetching the page HTML for any /co\d+ links
            const pageText = await page.content()
            const matches = [...pageText.matchAll(/href="(\/co\d+[^"]*?)"/g)].map((m) => `${SFB_BASE}${m[1]}`)
            listingUrls = [...new Set(matches)].slice(0, limit)
      ...
      if (listingUrls.length === 0) {
        ...
        await appendLog({ timestamp: new Date().toISOString(), source: 'sfbrandname', url: searchUrl, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, itemsExtracted: 0, error: 'No listing URLs found', errorType: null }).catch(() => {})
      ...
      for (let i = 0; i < listingUrls.length; i++) {
        ...
        try {
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'sfbrandname', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: isTimeout ? 504 : 500, screenshotFile: null, itemsExtracted: null, error: msg, errorType: isTimeout ? 'timeout' : 'screenshot' }).catch(() => {})
        ...
        try {
          ...
          try {
            ...
            await Promise.all([
              appendLog({ timestamp: ts, source: 'sfbrandname', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: result.items.length, error: null, errorType: null }).catch(() => {}),
              result.items.length > 0 ? saveResult({ timestamp: ts, source: 'sfbrandname', url, categoryId, screenshotFile: filename, items: result.items }).catch((e) => emit('warn', 'saveResult failed', String(e))) : Promise.resolve(),
            ])
          } catch {
            result.raw = text
            result.extractOk = false
            emit('warn', `[${i + 1}] Gemini response not valid JSON`, { preview: text.slice(0, 120) })
            await appendLog({ timestamp: new Date().toISOString(), source: 'sfbrandname', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: 0, error: `JSON parse failed: ${text.slice(0, 120)}`, errorType: 'parse' }).catch(() => {})
          }
        } catch (err) {
          const msg = err instanceof Error ? err.message : String(err)
          result.error = (result.error ?? '') + `extract: ${msg}`
          emit('error', `[${i + 1}] Gemini failed`, { msg })
          await appendLog({ timestamp: new Date().toISOString(), source: 'sfbrandname', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, itemsExtracted: null, error: msg, errorType: 'extraction' }).catch(() => {})
        }
