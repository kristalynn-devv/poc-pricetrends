...
export default defineEventHandler(async (event) => {
  const { query, categoryId = '108', template, limit, screenshotConfig: screenshotConfigRaw, roundId } = await readBody<{
  ...
  ;(async () => {
    ...
    try {
      ...
      if (listingUrls.length === 0) {
        ...
        await appendLog({ timestamp: new Date().toISOString(), source: 'sfbrandname', url: searchUrl, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, error: 'No listing URLs found', errorType: null }).catch(() => {})
      ...
      for (let i = 0; i < listingUrls.length; i++) {
        ...
        try {
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'sfbrandname', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: isTimeout ? 504 : 500, screenshotFile: null, error: msg, errorType: isTimeout ? 'timeout' : 'screenshot' }).catch(() => {})
        ...
        try {
          ...
          try {
            ...
            await Promise.all([
              appendLog({ timestamp: ts, source: 'sfbrandname', url, categoryId, searchQuery: query, roundId, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, dataFile, error: null, errorType: null }).catch(() => {}),
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'sfbrandname', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, error: msg, errorType: 'extraction' }).catch(() => {})
        }
