    ...
    try {
      ...
      try {
        ...
        if (detailUrls.length === 0) {
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'wutdychonburi', url: `${WDC_BASE}/product.php?menu=product-all&page=1`, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, error: 'No product links found', errorType: null }).catch(() => {})
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                await Promise.all([
                  appendLog({ timestamp: ts, source: 'wutdychonburi', url, categoryId, searchQuery: query, roundId, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, dataFile, error: null, errorType: null, geminiInputTokens, geminiOutputTokens, imageWidth, imageHeight }).catch(() => {}),
                  appendResult({ timestamp: ts, source: 'wutdychonburi', url, categoryId, screenshotFile: filename, items: result.items as Record<string, any>[], roundId, searchQuery: query }).catch((e) => emit('warn', 'appendResult failed', String(e))),
                ])
              } catch {
                result.raw = text
              ...
              await appendLog({ timestamp: new Date().toISOString(), source: 'wutdychonburi', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, error: msg, errorType: 'extraction' }).catch(() => {})
            }
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err)
