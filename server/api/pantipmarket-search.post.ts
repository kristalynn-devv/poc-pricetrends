...
const SEARCH_BASE = 'https://www.pantipmarket.com'
...
function buildFilename(index: number): string {
  ...
  return `${date}_107_PTM_${String(index + 1).padStart(2, '0')}.jpg`
}
...
export default defineEventHandler(async (event) => {
  const { query, categoryId = '107', template, limit = 5 } = await readBody<{
  ...
  const extractPrompt = `คุณคือผู้ช่วยสกัดข้อมูลสินค้ามือสองจากภาพหน้าเว็บ pantipmarket.com
...
${schemaText}
  ...
  const emit = (level: 'info' | 'warn' | 'error', msg: string, data?: unknown) => {
    ...
    const prefix = `[pantipmarket][${level.toUpperCase()}]`
  ...
  // Pantipmarket search URL: /search/?keyword=...&show_adv=0
  const searchUrl = `${SEARCH_BASE}/search/?keyword=${encodeURIComponent(query)}&show_adv=0`
  ...
  ;(async () => {
    ...
    try {
      ...
      try {
        ...
        // Pantipmarket item URLs are /items/XXXXXXX
        const allHrefs = await page.locator('a[href*="/items/"]').evaluateAll(
          (els) => [...new Set((els as HTMLAnchorElement[]).map((a) => a.href).filter((h) => /\/items\/\d+/.test(h)))]
        )
        const detailUrls = allHrefs.slice(0, limit)
        emit('info', `Found ${allHrefs.length} item links, processing ${detailUrls.length}`)

        if (detailUrls.length === 0) {
          emit('warn', 'No item links found')
          await appendLog({ timestamp: new Date().toISOString(), source: 'pantipmarket', url: searchUrl, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, itemsExtracted: 0, error: 'No item links found', errorType: null }).catch(() => {})
          send({ type: 'done', query, summary: { total: 0, screenshotOk: 0, extractOk: 0 }, error: 'No item links found' })
          ctrl.close()
          await ctx.close()
          await browser.close()
          return
          ...
          try {
            ...
            try {
              ...
              try {
                ...
                await Promise.all([
                  appendLog({ timestamp: ts, source: 'pantipmarket', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: result.items.length, error: null, errorType: null }).catch(() => {}),
                  result.items.length > 0 ? saveResult({ timestamp: ts, source: 'pantipmarket', url, categoryId, screenshotFile: filename, items: result.items }).catch((e) => emit('warn', 'saveResult failed', String(e))) : Promise.resolve(),
                ])
              } catch {
                result.raw = text
                result.extractOk = false
                emit('warn', `[${i + 1}] Gemini response not valid JSON`, { preview: text.slice(0, 120) })
                await appendLog({ timestamp: new Date().toISOString(), source: 'pantipmarket', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 200, screenshotFile: filename, itemsExtracted: 0, error: `JSON parse failed: ${text.slice(0, 120)}`, errorType: 'parse' }).catch(() => {})
              }
            } catch (err) {
              const msg = err instanceof Error ? err.message : String(err)
              result.error = `extract: ${msg}`
              emit('error', `[${i + 1}] Gemini failed`, { msg })
              await appendLog({ timestamp: new Date().toISOString(), source: 'pantipmarket', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, itemsExtracted: null, error: msg, errorType: 'extraction' }).catch(() => {})
            }
          } catch (err) {
            const msg = err instanceof Error ? err.message : String(err)
            ...
            await appendLog({ timestamp: new Date().toISOString(), source: 'pantipmarket', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: isTimeout ? 504 : 500, screenshotFile: null, itemsExtracted: null, error: msg, errorType: isTimeout ? 'timeout' : 'screenshot' }).catch(() => {})
          }
