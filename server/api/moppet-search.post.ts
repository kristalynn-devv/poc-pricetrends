    ...
    try {
      ...
      try {
        ...
        if (detailUrls.length === 0) {
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'moppet', url: searchUrl, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, error: 'No product links found', errorType: 'notfound' }).catch(() => {})
...
await persistExtraction({
  ...
  searchQuery: query, roundId, geminiInputTokens, geminiOutputTokens, imageWidth, imageHeight,
              ...
              await appendLog({ timestamp: new Date().toISOString(), source: 'moppet', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, error: msg, errorType }).catch(() => {})
            }
          } catch (err) {
            const { errorType, message: msg } = classifyError(err, 'screenshot')
            result.error = `screenshot: ${msg}`
            emit('error', `[${i + 1}] Failed`, { msg })
            await appendLog({ timestamp: new Date().toISOString(), source: 'moppet', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: errorType === 'timeout' ? 504 : 500, screenshotFile: null, error: msg, errorType }).catch(() => {})
          } finally {
            await itemCtx.close()
          }
