...
function buildFilename(index: number): string {
  return buildScreenshotFilename('111', 'T2H', String(index + 1).padStart(2, '0'))
}
...
export default defineEventHandler(async (event) => {
  const { query, categoryId = '111', template, limit, config: configRaw, roundId } = await readBody<{
  ...
  const { stream, send, emit, close } = createStreamEmitter('truck2hand')
  const geminiModel = createGeminiModel(apiKey)
  const screenshotDir = join(process.cwd(), 'output', 'screenshots')
  const results: ItemResult[] = []

  const searchUrl = `${SEARCH_BASE}?q=${encodeURIComponent(query)}`
    ...
    try {
      ...
      try {
        ...
        if (detailUrls.length === 0) {
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'truck2hand', url: searchUrl, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, error: 'No listing links found', errorType: 'notfound' }).catch(() => {})
          send({ type: 'done', query, summary: { total: 0, screenshotOk: 0, extractOk: 0 }, error: 'No listing links found', errorType: 'notfound' })
                    await ctx.close()
          await browser.close()
          return
        }
...
await persistExtraction({
  timestamp: ts, source: 'truck2hand', url, categoryId, screenshotFile: filename,
  ...
  searchQuery: query, roundId, geminiInputTokens, geminiOutputTokens, imageWidth, imageHeight,
              ...
              await appendLog({ timestamp: new Date().toISOString(), source: 'truck2hand', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, error: msg, errorType }).catch(() => {})
            }
          } catch (err) {
            const { errorType, message: msg } = classifyError(err, 'screenshot')
            result.error = `screenshot: ${msg}`
            emit('error', `[${i + 1}] Failed`, { msg })
            await appendLog({ timestamp: new Date().toISOString(), source: 'truck2hand', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: errorType === 'timeout' ? 504 : 500, screenshotFile: null, error: msg, errorType }).catch(() => {})
          } finally {
            await itemCtx.close()
          }
