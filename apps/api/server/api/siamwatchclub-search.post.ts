...
defineRouteMeta({
  openAPI: {
    tags: ['Batch Search'],
    summary: 'Search siamwatchclub.com',
    description: 'Scrapes a siamwatchclub.com listing page for query, screenshots + Gemini-extracts each matching item, and persists successes immediately to output/results/ + output/logs/. หมวด (categoryId): 103. Response is a stream of NDJSON lines (application/x-ndjson).',
    requestBody: {
      required: true,
      content: {
        'application/json': {
  ...
  ;(async () => {
    ...
    try {
      ...
      if (listingUrls.length === 0) {
        ...
        await appendLog({ timestamp: new Date().toISOString(), source: 'siamwatchclub', url: brandUrl, categoryId, searchQuery: query, durationMs: 0, httpStatus: 404, screenshotFile: null, error: 'No listing URLs found', errorType: 'notfound' }).catch(() => {})
      ...
      await runConcurrently(listingUrls.map((url, i) => async () => {
        ...
        try {
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'siamwatchclub', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: errorType === 'timeout' ? 504 : 500, screenshotFile: null, error: msg, errorType }).catch(() => {})
...
await persistExtraction({
  ...
  searchQuery: query, roundId, geminiInputTokens, geminiOutputTokens, imageWidth, imageHeight,
          ...
          await appendLog({ timestamp: new Date().toISOString(), source: 'siamwatchclub', url, categoryId, searchQuery: query, durationMs: Date.now() - itemStart, httpStatus: 502, screenshotFile: filename, error: msg, errorType }).catch(() => {})
        }
